				...
				<el-form-item label="书籍图片" prop="image">
					...
					<viewer>
						<img v-if="bookDetail.image" class="pic-area"
						     :src="$http.defaults.baseURL + bookDetail.image" style="width: 450px;"/>
						<!--'../assets/img/background.jpg'-->
					</viewer>
					<template v-if="!formDisable">
						<upload button-txt="上传图片" v-if="$root.$data.canEdit&&!bookDetail.image" path="/book/books/detail/"
						        :urlPath="'/book/books/detail/upload?id='+bookDetail.bookId"
						        style="width: 60%;" @uploadSuccess="handleFileUploadSuccess">
						</upload>
						<upload button-txt="更新图片" v-if="$root.$data.canEdit&&bookDetail.image" path="/book/books/detail/"
						        :urlPath="'/book/books/detail/upload?id=' + bookDetail.bookId"
						        style="width: 60%;" @uploadSuccess="handleFileUploadSuccess">
						</upload>
					</template>
...
export default {
	...
	methods: {
		async getBookDetail(){
			...
			const {data: res}=await this.$http.get("/book/books/detail/"+this.bookId);
			if(res.code!==200){
				this.pageStatus=0;
				return;
			}
